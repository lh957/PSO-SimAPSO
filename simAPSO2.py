...
class SIMAPSO(object):
    ...
    def random_pgp(self):
        ...
        fitIndex = -(self.piv - groupFit) / self.T
        fitIndex = MaxAbsScaler().fit_transform(X=fitIndex.reshape(1, -1)) * 100
        Tfit = 0.0001 + np.exp(fitIndex)
        Tfit = Tfit / np.nansum(Tfit)
        # 用轮盘赌策略确定全局最优的某个替代值
        p_best = np.abs(np.random.randn(1)[0])
        ComFit = np.zeros(self.n_particles)
        for ipt in range(self.n_particles):
            ComFit[ipt] = np.nansum(Tfit[:ipt])
...
if __name__ == '__main__':
    simapso = SIMAPSO(holdertable, n_particles=100, dimensions=2, c1=0.5, c2=0.3, bounds=([-10, -10], [10, 10]))
